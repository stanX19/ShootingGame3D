...
void ecs_system::player_update(entt::registry& registry, float dt) {
    ...
    for (auto entity : view) {
        ...
        if (IsKeyDown(KEY_RIGHT) || IsKeyDown(KEY_D)) rotation.value.y -= turnSpeed;
        if (IsKeyDown(KEY_LEFT) || IsKeyDown(KEY_A))  rotation.value.y += turnSpeed;
